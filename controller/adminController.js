...
module.exports = {
    ...
    adminLogin: async (req, res, next) => {
        try {
            ...
            // await bcrypt.compare(password, '7014785432')
            if (!isCorrect) {
    ...
    getAllComplaints: async (req, res, next) => {
        try {
            const complaints = await Complaint.find().sort({ date: 'desc' }); // sorts complaints by date in descending order
...
,
    postAnnouncements: async (req, res, next) => {
        // Validate the request body
        if (!req.body.title || !req.body.description) {
            return res.status(400).send({error: 'Title and Description are required fields'});
            }
